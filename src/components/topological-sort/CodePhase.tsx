...
import { topologicalSortCode, topologicalSortBridge } from "@/lib/traces/topologicalSort";

export default function TopologicalSortCodePhase() {
  ...
  const highlightLines = highlightedLine
    ...
    ? [topologicalSortBridge[highlightedMathIdx].codeLine]
    : [];
  ...
  return (
    <section className="mb-16">
      <PhaseHeader
        phase={3}
        title="The Code"
        subtitle="Bridge from DFS finish-time ordering to Python implementation"
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2">
              {topologicalSortBridge.map((item, idx) => (
                <div
                  key={idx}
          ...
          <div>
            ...
            <CodeBlock
              code={topologicalSortCode}
              highlightLines={highlightLines}
              onLineHover={(line) => {
                setHighlightedLine(line);
                if (line) {
                  const idx = topologicalSortBridge.findIndex((b) => b.codeLine === line);
